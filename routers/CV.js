  ...
  (req, res) => {
    ...
    CV.find({ writer })
      ...
      .exec((err, result) => {
        ...
        CV.countDocuments({ writer }, (err, total) => {
          if (err) {
            console.error("Error counting CVs:", err);
            return res.status(400).json({
              success: false,
              message: { msgBody: "Lỗi đếm CV!!!", msgError: true },
              err,
            });
          }
          res.status(200).json({
            success: true,
            message: { msgBody: "Load dữ liệu thành công", msgError: false },
            result,
            total,
          });
        });
...
cVRouter.post(
  ...
  (req, res) => {
    ...
    CV.findOne({ _id: idCV, receiver: _id })
      .populate("recruitment", "title")
      .exec((err, result) => {
        if (err) {
          res.status(400).json({
