...
recruitmentRouter.post(
  ...
  passport.authenticate("jwt", { session: false }),
  (req, res) => {
    const { role, _id, hasUsedFreePost, status } = req.user;
    ...
    if (role !== "recruiter") {
      return res.status(403).json({
        success: false,
        message: {
          msgBody: "Chỉ nhà tuyển dụng mới có thể đăng bài tuyển dụng",
          msgError: true,
        },
      });
    }
    ...
    if (!status) {
      return res.status(203).json({
        success: false,
        message: {
          msgBody: "Tài khoản này đã bị khóa",
          msgError: true,
        },
      });
    }
    ...
    // Kiểm tra xem có bài viết đang chờ phê duyệt hay không
    Recruitment.find({ writer: writer, status: false }, (err, result) => {
      if (err) {
      ...
      // Kiểm tra quyền đăng bài (lượt miễn phí hoặc gói đăng ký)
      if (!hasUsedFreePost) {
        // Cho phép đăng bài miễn phí lần đầu
        const newRrm = new Recruitment({
          email,
          sdt,
          contact,
        ...
        newRrm.save((err, result) => {
          ...
          // Cập nhật trạng thái đã sử dụng lượt miễn phí
          Account.findByIdAndUpdate(_id, { hasUsedFreePost: true }, (err) => {
            if (err) {
              return res.status(500).json({
                success: false,
                message: {
                  msgBody: "Lỗi khi cập nhật trạng thái lượt miễn phí",
                  msgError: true,
                },
                err,
              });
            ...
            // Cập nhật liên kết với City và Career
            City.updateOne(
              { _id: city },
              { $push: { recruitments: newRrm } },
              (err) => {
            ...
            return res.status(200).json({
              message: {
                msgBody: "Thêm dữ liệu thành công (lượt miễn phí)",
                msgError: false,
              },
              result,
            });
          ...
          (err, subscription) => {
            if (err || !subscription) {
              return res.status(403).json({
                success: false,
                message: {
                  msgBody: "Bạn cần mua gói đăng ký để tiếp tục đăng bài",
                  msgError: true,
                },
              });
            }

            // Kiểm tra số bài đã sử dụng trong gói
            SubscriptionPackage.findById(
              ...
              (err, package) => {
                if (err || !package) {
                  return res.status(500).json({
                    success: false,
                    message: {
                      msgBody: "Lỗi khi kiểm tra gói đăng ký",
                      msgError: true,
                    },
                    err,
                  });
                ...
                if (subscription.postsUsed >= package.postLimit) {
                  return res.status(403).json({
                    success: false,
                    message: {
                      msgBody:
                        "Bạn đã sử dụng hết số bài đăng trong gói, vui lòng mua gói mới",
                      msgError: true,
                    },
                  });
                }
                ...
                newRrm.save((err, result) => {
                  ...
                  // Cập nhật số bài đã sử dụng trong gói
                  PurchasedSubscription.findByIdAndUpdate(
                    subscription._id,
                    { $inc: { postsUsed: 1 } },
                    { new: true },
                    (err) => {
                      if (err) {
                        return res.status(500).json({
                          success: false,
                          message: {
                            msgBody: "Lỗi khi cập nhật số bài đã sử dụng",
                            msgError: true,
                          },
                          err,
                        });
